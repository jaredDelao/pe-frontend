...
<template>
  <div class="grid min-h-screen lg:grid-cols-2">
    <div class="relative hidden overflow-hidden lg:block">
      ...
      <div class="absolute inset-0 bg-linear-to-t from-brand-900/85 via-brand-900/45 to-brand-900/25" />

      <div class="relative z-10 flex h-full flex-col justify-between p-12 text-white">
        <div class="flex items-center gap-2.5">
          <img :src="logoImage" alt="Pagos PE" class="size-10 rounded-xl object-contain" />
    ...
    <div class="flex items-center justify-center p-6">
      <div class="w-full max-w-sm animate-fade-up">
        ...
        <form class="flex flex-col gap-4" @submit.prevent="onSubmit">
          <BaseInput v-model="username" label="Usuario" placeholder="supervisor / operador" autocomplete="username" />

          <BaseInput v-model="password" label="Contraseña" :type="showPassword ? 'text' : 'password'"
            placeholder="••••••••" autocomplete="current-password">
            <template #suffix>
              <button type="button" class="text-slate-400 transition hover:text-slate-600" :aria-label="showPassword ? 'Ocultar contraseña' : 'Mostrar contraseña'
          ...
          <BaseButton type="submit" :loading="loading" :disabled="!canSubmit" block class="mt-2">
            Entrar
          </BaseButton>
        </form>

        <p class="mt-6 rounded-xl bg-slate-50 p-3 text-xs text-slate-500 dark:bg-slate-800 dark:text-slate-400">
          <span class="font-medium text-slate-600 dark:text-slate-300">Acceso de prueba:</span>
